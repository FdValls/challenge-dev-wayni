"use client";

import { MainLayoutProps } from "@/interfaces/LayoutHomeProps";

function SendAgainTwoLayout({ bodyContent, footerContent }: MainLayoutProps) {
    return (
        <div className="flex flex-col items-center justify-center  max-h-screen p-4">
            {/* Mobile Container */}
            <div className="w-[400px] h-[756px] bg-emerald-400 overflow-hidden ">
                {/* Main Content Card */}
                <div className="mx-6 mt-8 bg-white rounded-2xl overflow-hidden shadow-sm flex-1 mb-2">
                    <div className="p-6 h-[390px]">{bodyContent}</div>
                </div>

                {/* Bottom Action Area */}
                <div className="px-6 pb-8 ">{footerContent}</div>
            </div>
        </div>
    );
}

export default SendAgainTwoLayout;
